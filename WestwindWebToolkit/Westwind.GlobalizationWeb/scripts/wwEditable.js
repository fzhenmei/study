...
function wwEditable(ctl,options) {
    ...
    this.editClass = null;
    this.editMode = "text";  // "text","html","formatted"
    ...
    {        
        ...
        else if(_I.editMode == "formatted")
        {
            ...
            txt = $("<div>" + $.trim(txt) + "</div>").text().replace(/#CRLF#/g,"\r\n");  
            _I.origText = txt;      
        }
        else
        ...
        if (_I.textMode == "text")       
            _I.jcol.html("<" +"form action='javascript:void(0);'><input id='__editColumn' type='text' value='" + _I.origText + "'/></form>");
        else
